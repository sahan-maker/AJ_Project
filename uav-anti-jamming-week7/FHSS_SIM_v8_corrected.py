...
class SystemIntegrationCanvas(FigureCanvas):
    ...
    def _draw_scorecard(self):
        ...
        # Separator (drawn in axes coordinates)
        ax.add_line(Line2D([0, 1], [0, 0], transform=ax.transAxes,
                   color=BORDER, lw=0.5))
...
class JammingAnalysisCanvas(FigureCanvas):
    ...
    def _redraw_timeline(self):
        ...
        # Use compact x-axis (0..N-1) for plotting so large absolute hop
        # indices don't push the bars off-screen. Show absolute hop numbers
        # as tick labels instead.
        N = len(hops)
        x = np.arange(N)

        # Persistent twin axis: reuse or recreate to avoid stacking axes
        ...
        # Colors per bar based on channel index
        bar_colors = [CHANNEL_COLORS[c % len(CHANNEL_COLORS)] for c in chs]

        ax.bar(x, bers, color=bar_colors, alpha=0.8, width=0.75, zorder=3)
        if len(snrs) > 0:
            ax2.plot(x, snrs, color=YELLOW, lw=1.2, alpha=0.9, zorder=4)
        ...
        # X ticks: show up to 8 labels evenly spaced with absolute hop numbers
        if N > 0:
            max_labels = 8
            step = max(1, N // max_labels)
            tick_pos = x[::step]
            tick_labels = [str(h) for h in hops[::step]]
            ax.set_xticks(tick_pos)
            ax.set_xticklabels(tick_labels, color=MUTED, fontsize=7)
        ax.set_ylim(0, 1.05)
        ax.grid(color=BORDER, lw=0.35, alpha=0.5)
...
class TXPanel(QGroupBox):
    ...
    def __init__(self, engine: FHSSEngine):
        ...
        # Ensure message input remains wide and readable
...
class FHSSWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self._make_placeholder_tab("🏗  System", self._create_system_tab)
        ...
        rv.addStretch()
        # Make right column scrollable to avoid vertical compression when fullscreen
    ...
    def _on_tab_changed(self, index: int):
        ...
        try:
            ...
            traceback.print_exc()
            err_lbl = QLabel("Error loading tab content. See console for traceback.")
            err_lbl.setStyleSheet(f"color:{RED};font-size:11px;")
