...
==================
Week 4 additions:
  • Spectrum Sensing via Energy Detection (per-channel energy computation each hop)
  • Channel Occupancy Detection (energy vs adaptive threshold → occupied/free)
  • Channel Energy Analysis tab:
      - Live per-channel energy bar chart with threshold line
      - Occupancy heatmap (channel × hop history)
      - Detection performance stats (Pd, Pfa, miss rate vs ground-truth jam)
...
Prior weeks:
  ...
  4 jammer types: Barrage, Spot, Partial-Band, Sweep
  Per-channel jam targeting, Jamming Analysis tab
...
class StatsBar(QWidget):
    def __init__(self, engine: FHSSEngine):
        ...
        self._lbs = {}
        for key in ["Channels","Hop Rate","Total BW","Modulation","SNR","Jamming","Jam Type","Sensing"]:
            fr = QFrame()
...
class FHSSWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(
            "FHSS Simulation v5  —  Week 4: Spectrum Sensing · Energy Detection · Occupancy Analysis")
    ...
    def _build_ui(self):
        ...
        hdr = QLabel("⟁  FHSS Simulation v5  ·  Message TX/RX  ·  Noise + Jamming  ·  Spectrum Sensing  ·  Week 4")
        ...
        self.ch_panel.changed.connect(lambda: self.stats.refresh(self.detector))
    ...
    def _on_sensing_changed(self):
        # Redraw energy tab without resetting history
    ...
    def _on_config_changed(self):
        ...
        self.stats.refresh(self.detector)
    ...
    def _on_hop(self, hop_idx: int):
        ...
        # ── Generate IQ per-hop for ALL channels (for sensing) ────────────────
        # We synthesise a short probe signal on each channel so the energy
        # detector always has fresh data even on non-transmit hops.
        probe_len = 256
        all_rx_iqs = []
        ...
        # Run energy detector on all channels
        ...
        active_sense = sense_results[ch_idx]
