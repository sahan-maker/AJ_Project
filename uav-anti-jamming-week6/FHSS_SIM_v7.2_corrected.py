...
from PyQt5.QtWidgets import (
    ...
    QSplitter, QTabWidget, QTextEdit, QLineEdit, QCheckBox, QGridLayout
)
...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QButtonGroup, QScrollArea, QGridLayout, QSizePolicy
)
from PyQt5.QtCore import Qt, QTimer, pyqtSignal
from PyQt5.QtGui import QFont, QColor, QPalette, QTextCursor
...
class JammingAnalysisCanvas(FigureCanvas):
    ...
    def __init__(self, engine: "FHSSEngine", parent=None):
        ...
        self._ch_history = []
        self._snr_history = []
        self._ax2 = None  # Persistent twin axis
    ...
    def _redraw_bar(self):
        ...
        n = eng.num_channels
        xs = np.arange(n)
        colors = [RED if eng.is_channel_jammed(i) else GREEN+"90" for i in range(n)]
        if eng.jam_db <= -60:
            colors = [MUTED+"60"] * n

        ax.bar(xs, self._jam_exposure[:n], color=colors, width=0.7, zorder=3)
    ...
    def _redraw_timeline(self):
        ...
        # Persistent twin axis
        if self._ax2 is None or self._ax2 not in ax.figure.axes:
            if self._ax2 is not None:
                try: self._ax2.remove()
                except: pass
            self._ax2 = ax.twinx()
        else:
            self._ax2.clear()
        ...
        ax2.tick_params(colors=YELLOW, labelsize=7)

        x = list(range(len(hops)))
        bar_colors = [CHANNEL_COLORS[c % len(CHANNEL_COLORS)] for c in chs]

        ax.bar(x, bers, color=bar_colors, alpha=0.75, width=0.75, zorder=3)
        ax2.plot(x, snrs, color=YELLOW, lw=1.4, alpha=0.9, zorder=4)

        ax.set_ylabel("BER", color=MUTED, fontsize=7.5)
        ax2.set_ylabel("SNR meas (dB)", color=YELLOW, fontsize=7.5)
        ax.set_xlabel("Hop # (recent)", color=MUTED, fontsize=7.5)

        ax.set_xlim(-0.5, len(hops) - 0.5)
        ax.set_ylim(0, 1.05)
        ax.grid(color=BORDER, lw=0.35, alpha=0.5)
        ax.axhline(0.1, color=RED, lw=0.8, ls="--", alpha=0.6)
    ...
    def _redraw_status(self):
        ...
        for i in range(n):
            ...
            ax.add_patch(mpatches.Circle((x, y), 0.04, transform=ax.transAxes, color=col, zorder=3))
    ...
    def update_hop(self, hop_idx: int, ch_idx: int, ber: float, snr_meas: float):
        ...
        # Fixed: enforce max history
        if len(self._ber_history) > self.MAX_HISTORY:
            self._ber_history.pop(0)
            self._snr_history.pop(0)
            self._hop_history.pop(0)
            self._ch_history.pop(0)

        eng.advance_sweep()
        self._redraw_all()

    def reset_stats(self):
        self._jam_exposure = np.zeros(self.engine.num_channels)
        self._ber_history.clear()
        self._snr_history.clear()
        self._hop_history.clear()
...
class TXPanel(QGroupBox):
    ...
    def __init__(self, engine: FHSSEngine):
        ...
        # Keep modulation control readable when right pane narrows
        try:
            self.cb_mod.setMinimumWidth(140)
            self.cb_mod.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            self.cb_mod.setMinimumHeight(28)
        except Exception:
            pass
        lay.addLayout(ml)
        ...
        try:
            self.msg_input.setMinimumWidth(260)
            self.msg_input.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            self.msg_input.setMinimumHeight(28)
        except Exception:
            pass
...
class HopSettingsPanel(QGroupBox):
    ...
    def __init__(self, engine: FHSSEngine):
        ...
        # Make FHSS setting widgets maintain readable width
        for w in (self.sb_ch, self.dsb_bf, self.dsb_bw, self.sb_hi, self.sb_sl, self.sb_sd):
            try:
...
class FHSSWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(
            "FHSS Simulation v7  —  Week 6: Predictive Channel Selection · History Tracking · Accuracy Evaluation")
        ...
        self.adaptive_eng.predictor = self.predictor   # wire predictor in
        self.perf_tracker  = PerformanceTracker()
        self.hop_map = None
        self.jam_analysis = None
    ...
    def _build_ui(self):
        ...
        hdr = QLabel("⟁  FHSS Simulation v7  ·  TX/RX  ·  Jamming  ·  Sensing  ·  Adaptive  ·  Predictive  ·  Week 6")
        ...
        self._make_placeholder_tab("📶  Hop Map", self._create_hop_map_tab)
        self._make_placeholder_tab("⚡  Jamming Analysis", self._create_jam_analysis_tab)
        self._make_placeholder_tab("🔍  Energy / Sensing", self._create_energy_analysis_tab)
        self._make_placeholder_tab("📊  Performance", self._create_perf_canvas_tab)
        self._make_placeholder_tab("🔮  Prediction", self._create_pred_canvas_tab)
        ...
        rv.addStretch()
        # Wrap right column in scroll area to avoid vertical compression
        scroll = QScrollArea(); scroll.setWidgetResizable(True); scroll.setWidget(right)
        scroll.setMinimumWidth(360)
        msplit.addWidget(scroll)
        # Prefer left pane to take most space; reserve right pane minimum width
        msplit.setStretchFactor(0, 3)
        msplit.setStretchFactor(1, 1)
        msplit.setSizes([1040, 420])
        root.addWidget(msplit, 1)

    def _make_placeholder_tab(self, title, create_fn):
        tab = QWidget()
        layout = QVBoxLayout(tab)
        layout.setContentsMargins(4,4,4,4)
        label = QLabel("Tab content will load when selected.")
        ...
        self._tab_factories[idx] = create_fn
        return tab

    def _on_tab_changed(self, index: int):
        if index <= 0 or index not in self._tab_factories:
            return
        create_fn = self._tab_factories.pop(index)
    ...
    def _on_hop(self, hop_idx: int):
        ...
        # Week 6: update history and predictor BEFORE jam_analysis so predictor
        # has fresh data for next hop's channel selection
