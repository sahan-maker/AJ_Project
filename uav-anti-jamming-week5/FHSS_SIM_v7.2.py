"""
FHSS Simulation v7.2
==================
Week 6 additions:
  • ChannelHistoryTracker  — rolling per-channel history of energy, occupancy, BER
      - Tracks N×T energy matrix, per-channel jam probability, mean energy, trend
  • ChannelPredictor       — three independent prediction models:
      - "Markov"              : first-order Markov chain, P(occ_t+1 | occ_t / free_t)
      - "Exp Smoothing"       : EWMA of occupancy signal → smooth probability forecast
      - "Trend"               : linear regression over recent energy → extrapolated probability
      All three run simultaneously; accuracy compared on every hop.
  • Predictive channel selection — AdaptiveFHSSEngine can use predicted occupancy
    probabilities (instead of / combined with current snapshot) for forward-looking
    channel selection ("Predictive" strategy).
  • Prediction tab (🔮) — four live plots:
      - Per-channel predicted occupancy probability vs actual occupancy
      - Prediction accuracy timeline (rolling 16-hop correct-prediction rate)
      - Channel history heatmap (energy + occupancy + prediction overlaid)
      - Model comparison bar chart (accuracy / precision / recall for all 3 models)
  • PredictivePanel — right-sidebar: model selector, enable toggle, live accuracy readout

Week 5 retained:
  • Adaptive FHSS (Avoid Jammed / Best Channel), Performance Comparison tab

Weeks 1-4 retained:
  • 5 modulations, TX/RX pipeline, 4 jammer models, Jamming Analysis, Energy/Sensing
